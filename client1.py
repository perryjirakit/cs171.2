...
# ---------- Config ----------
# Fixed map matches the template defaults/grader expectations
...
client_state = "RELEASED"   # RELEASED | WANTED | HELD
...
current_insert = None  # (perm, grade)
...
# Lamport clock rules: only on send/recv of REQUEST
...
def have_all_replies() -> bool:
    with replies_received_lock:
        return len(replies_received) == 2  # two peers
...
def start_lamport_insert(perm: str, grade: str, master_conn: socket.socket):
    # Mark desire and reset barriers
    ...
    # Request timestamp + enqueue self
...
def handle_connection(conn: socket.socket, addr):
    ...
    try:
        while True:
            ...
            # Master commands (no artificial delay)
            if cmd == "insert":
...
def main():
    ...
    MY_PORT = args.port  # accept any, but peers use fixed map

    t = threading.Thread(target=run_server, args=(MY_PORT,), daemon=True)
    t.start()
    time.sleep(1.5)  # allow server to come up
